...
export class LinuxMouseStrategy implements IMouseStrategy {
  ...
  public async move(dx: number, dy: number): Promise<void> {
    ...
    const locationResult = await this.execFileAsync("xdotool", ["getmouselocation"], {
      shell: false,
    });

    const match = locationResult.stdout.match(/x:(-?\d+)\s+y:(-?\d+)/);
    if (!match) {
      throw new AppError("Failed to read current mouse position", 500, "MOUSE_POSITION_READ_FAILED");
    }
    ...
    await this.execFileAsync("xdotool", ["mousemove", "--", String(nextX), String(nextY)], {
      shell: false,
    });
  }
  ...
  public async click(button: "left" | "right"): Promise<void> {
    ...
    const buttonCode = button === "left" ? "1" : "3";
    await this.execFileAsync("xdotool", ["click", buttonCode], { shell: false });
  }
  ...
  private checkDependencies(): boolean {
    try {
      ...
      this.logger.warn(
        "xdotool not found. Mouse control unavailable. Install: sudo apt install xdotool",
      );
      return false;
    }
  }
  ...
  private ensureAvailable(): void {
    ...
    throw new AppError(
      "Mouse control unavailable: xdotool is not installed",
      503,
